...
const plugin = {
  ...
  configs: {
    ...
    // Note: The 'flat/recommended' config is defined as a getter to avoid circular reference
    // Users should access it via: import plugin from 'eslint-plugin-react-pure-export'
    // then use: plugin.configs['flat/recommended']
    'flat/recommended': {
      // Don't include plugins here to avoid circular reference
      // Users need to add the plugin separately in their flat config
      rules: {
        'react-pure-export/no-non-component-export-in-tsx': 'error',
        'react-pure-export/no-tsx-import-in-pure-module': 'error',
        'react-pure-export/no-heavy-deps-in-pure-module': 'error'

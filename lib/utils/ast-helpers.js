        ...
        if (node.declaration) {
            ...
            if (node.declaration.type === 'VariableDeclaration') {
                if (node.declaration.declarations && node.declaration.declarations.length > 0) {
                    ...
                    if (declarator.id) {
                        // Handle simple identifier: export const NAME = ...
                        if (declarator.id.type === 'Identifier' && declarator.id.name) {
                            return declarator.id.name;
                        }
                        // Handle object destructuring: export const { prop: NAME } = ...
                        if (declarator.id.type === 'ObjectPattern' && declarator.id.properties && declarator.id.properties.length > 0) {
                            const firstProperty = declarator.id.properties[0];
                            // Return the first exported name from destructuring
                            if (firstProperty.value && firstProperty.value.type === 'Identifier' && firstProperty.value.name) {
                                return firstProperty.value.name;
                            }
                            if (firstProperty.key && firstProperty.key.type === 'Identifier' && firstProperty.key.name) {
                                return firstProperty.key.name;
                            }
                        }
                        // Handle array destructuring: export const [NAME] = ...
                        if (declarator.id.type === 'ArrayPattern' && declarator.id.elements && declarator.id.elements.length > 0) {
                            const firstElement = declarator.id.elements[0];
                            if (firstElement && firstElement.type === 'Identifier' && firstElement.name) {

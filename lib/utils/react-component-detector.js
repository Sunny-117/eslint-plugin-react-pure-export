...
function isWrappedWithHOC(node) {
    ...
    if (node.type === 'VariableDeclaration') {
        ...
        // Handle type assertion: withBoundary(...) as SomeType
        if (init.type === 'TSAsExpression' || init.type === 'TSTypeAssertion') {
            init = init.expression;
        }

        return isHOCPattern(init);
    }
...
function isReactMemo(node) {
    ...
    if (node.type === 'VariableDeclaration') {
        ...
        // Handle type assertion: memo(...) as SomeType
        if (init.type === 'TSAsExpression' || init.type === 'TSTypeAssertion') {
            init = init.expression;
        }
...
function isReactForwardRef(node) {
    ...
    if (node.type === 'VariableDeclaration') {
        ...
        // Handle type assertion: forwardRef(...) as SomeType
        if (init.type === 'TSAsExpression' || init.type === 'TSTypeAssertion') {
            init = init.expression;
        }
